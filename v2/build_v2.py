...
def main():
    ...
    vocab = w["vocab"]; V = len(vocab)
    c = w["config"]
    T=c["T"]; SLOTS=c["SLOTS"]; A1=c["A1"]
    B1=c["TURN1_BOT"]; T2S=c["TURN2_START"]; B2=c["TURN2_BOT"]
    D=c["D"]; L=c["L"]; QH=c["Q_HEADS"]; HD=c["HD"]; KVD=c["KV_DIM"]; MLP=c["MLP"]
    ...
    css.append("/* qwennie v2 — generated. pure CSS transformer runtime. */")
    ...
    input_ids=w.get("input_token_ids") or [i for i in range(4,V)]
    for turn in (1,2):
    ...
    for t in range(V):
        row=[f"--l0_x{j}:{fmt(Q['emb'][t][j]*S['emb'][j])}" for j in range(D)]
        css.append(f"@container style(--tk: {t}){{.cl{{{';'.join(row)}}}}}")
    css.append("")

    def matmul(dst, src, name, nin, nout):
        ...
        for j in range(nout):
            ...
            lines.append(f"  --{dst}{j}:calc({fmt(s[j])}*(0{''.join(terms)}));")
        return lines
    ...
    for t,tok in enumerate(vocab): css.append(f'@container style(--tk: {t}){{.g::before{{content:"{css_escape(glyph_text(tok))}"}}}}')
    css.append('@container style(--dn: 1){.g::before{content:""}}')
    ...
    def gen_wrapper(pos, inner, with_gc, turn):
        cls=f"cl p{pos}"+(" gc" if with_gc else "")
        return f'<span class="w gw gen t{turn}"><span class="g"></span><span class="{cls}">{inner}</span></span>'
    ...
    selects1="".join(f'<select id="u1s{s}">{options(d1,s)}</select>' for s in range(SLOTS))
    selects2="".join(f'<select id="u2s{s}">{options(d2,s)}</select>' for s in range(SLOTS))
